...
We denote $U$ as unknown parameters and $K$ as known parameters. The Bayes' Theorem gives us the posterior distribution of unknown parameters given the known parameters: 
$$ P(U \mid K) \propto P(U) \cdot P(K \mid U)$$

The $P(U)$ is prior and $P(K|U)$ is likelihood. Let $K = \left\{y_{n \times 1}, X_{n \times p} \right\}$ 
and assume $y \sim N\left( X \beta, \sigma^{2} V\right)$, 
where $V$ is known and $U = \left\{\beta, \sigma^{2}\right\}$ is unknown. 
...
## 2. Joint distribution of Normal Inverse-Gamma prior
We use a Normal Inverse-Gamma (NIG) prior: 
...
Note: $X \sim  Gamma(\alpha, \beta)$, the density function of $X$ is $f(x)=\frac{\beta^{\alpha}}{\Gamma(\alpha)} x^{\alpha-1} e^{-\beta x}$. Let $Y=\frac{1}{X} \sim IG(\alpha, \beta)$, the density function of $Y$ is $f(y)=\frac{\beta^{\alpha}}{\Gamma(\alpha)} x^{-\alpha-1} e^{-\frac{\beta}{x}}$.

## 3. Marginal distribution of Normal Inverse-Gamma prior
...
Note: the density of multivariate t-distribution is:
...
In summary, the marginal priors are:
...
From results in marginal priors, the marginal posterior distributions can be easily get by updating corresponding parameters: 
...
Let $\tilde{y}$ denote an $\tilde{n}\times 1$ vector of outcomes. We seek to predict it based upon predictors $\tilde{X}$. Assume $V=I_{n}$.
